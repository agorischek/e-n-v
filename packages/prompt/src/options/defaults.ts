...
export const SECRET_PATTERNS: ReadonlyArray<RegExp | string> = Object.freeze([
  ...
  /passphrase/i,
  /secret/i,
  /(?<!(?:(?:expires[_-]?in|expiry|expiration|ttl|lifetime|duration|age|max[_-]?age|validity)[_-]?))token(?![_-]?(?:expires|expiry|expiration|ttl|lifetime|duration|age|max[_-]?age|validity))/i,
  /api[_-]?key/i,
  /client[_-]?secret/i,
  /private[_-]?key/i,
