...
describe("Core preprocessing integration", () => {
  test("applyPreprocessing works with all schema types", () => {
    const preprocessorOptions = {
      ...
      bool: (value: string) => value.toLowerCase() === 'on' ? 'true' : 'false',
      enum: (value: string) => value.toLowerCase()
    };

    // Test string preprocessing
    expect(applyPreprocessing("  HELLO  ", "string", preprocessorOptions)).toBe("hello");

    // Test number preprocessing  
    expect(applyPreprocessing("1,000", "number", preprocessorOptions)).toBe("1000");

    // Test boolean preprocessing
    expect(applyPreprocessing("on", "boolean", preprocessorOptions)).toBe("true");
    expect(applyPreprocessing("OFF", "boolean", preprocessorOptions)).toBe("false");
  ...
  test("default preprocessors work for number and boolean", () => {
    ...
    expect(applyPreprocessing("10,000.50", "number", undefined)).toBe("10000.50");
  ...
  test("default preprocessors pass through unparseable values", () => {
    ...
    expect(applyPreprocessing("not-a-number", "number", undefined)).toBe("not-a-number");
    expect(applyPreprocessing("1,2,3,abc", "number", undefined)).toBe("1,2,3,abc");
    
    // Booleans that aren't recognized should pass through
    expect(applyPreprocessing("maybe", "boolean", undefined)).toBe("maybe");
    expect(applyPreprocessing("sometimes", "boolean", undefined)).toBe("sometimes");
  ...
  test("applyPreprocessing handles null/undefined preprocessors", () => {
    ...
    expect(applyPreprocessing("TEST", "string", preprocessorOptions)).toBe("TEST");
    expect(applyPreprocessing("1,000", "number", preprocessorOptions)).toBe("1000"); // Default applied
    expect(applyPreprocessing("TRUE", "boolean", preprocessorOptions)).toBe("true");
  });
  ...
  test("preprocessing can return target types directly", () => {
    ...
    const numberResult = applyPreprocessing<number>("1,000", "number", preprocessorOptions);
    expect(numberResult).toBe(1000);
    expect(typeof numberResult).toBe("number");

    const boolResult = applyPreprocessing<boolean>("on", "boolean", preprocessorOptions);
