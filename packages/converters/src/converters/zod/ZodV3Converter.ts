...
/**
 * Zod v3 schema converter implementation
 */
export class ZodV3Converter implements SchemaConverter<ZodTypeAny> {
  applies(schema: unknown): schema is ZodTypeAny {
    return isZodV3Schema(schema);
  }

  convert(schema: ZodTypeAny): EnvVarSchema {
 ...
 */
export function convertFromZodV3Schema(schema: ZodTypeAny): EnvVarSchema {
  ...
  switch (type) {
    case "boolean": {
      const process = createV3ProcessFunction<boolean>(
        peeled.schema,
        "boolean",
      );
